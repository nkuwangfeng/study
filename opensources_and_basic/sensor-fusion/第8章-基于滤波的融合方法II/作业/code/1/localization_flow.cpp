...
LocalizationFlow::LocalizationFlow(ros::NodeHandle& nh) {
    ...
    dv_noise_ = config_node["dv_noise"].as<double>();
    angular_rate_threshold_ = config_node["angular_rate_threshold"].as<double>();
    ...
    FileManager::CreateFile(velocity_ofs_, WORK_SPACE_PATH + "/localization_data/velocity.txt");
    velocity_ofs_.precision(12);
}
...
bool LocalizationFlow::InitGNSS() {
    ...
    if (!gnss_inited) {
        ...
        GNSSData gnss_data;
        gnss_data.latitude = 48.982602707;
        gnss_data.longitude = 8.39041175026;
        gnss_data.altitude = 116.389606807;
...
bool LocalizationFlow::Predict()
{
    ...
    for (unsigned int i = 1; i < current_imu_data_.size(); ++i)
    {
